...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.evaluate(()=>getComputedStyle(document.documentElement).getPropertyValue('--accent').trim()),'#ff77b7');
  await page.locator('#welcomeForm [type=submit]').click();
  await page.locator('#looksButton').click();assert.equal(await page.locator('#lookSelect').inputValue(),'df-rainbow');
  fs.mkdirSync(path.join(__dirname,'artifacts'),{recursive:true});await page.screenshot({path:path.join(__dirname,'artifacts/public-rainbow.png')});
