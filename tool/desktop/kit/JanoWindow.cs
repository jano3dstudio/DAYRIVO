...
public class JanoWindow : Form {
    public const string ChromeRevision="0.2.0-chrome.2";
    ...
    protected override void WndProc(ref Message m){
        // WinForms Click can run while its caption button still owns capture.
        // Retry the system command after MouseUp; handler-only tests miss this.
        if(m.Msg==0x112){
            int command=m.WParam.ToInt32()&0xfff0;
            var captured=Control.FromHandle(GetCapture());
            if((command==0xF020||command==0xF030||command==0xF120)&&captured is Button&&captured.FindForm()==this){
                var w=m.WParam;var l=m.LParam;
                BeginInvoke(new Action(()=>{if(!IsDisposed&&IsHandleCreated)SendMessage(Handle,0x112,w,l);}));
